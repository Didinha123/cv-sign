...
// ─────────────────────────────────────────────────────────────
// PRODUTOS (hardcoded — preços atualizados pelo Sheets)
// ─────────────────────────────────────────────────────────────
const PRODUCTS = {
...
async function carregarPrecosDoSheets() {
  ...
  if (!url) return;
  try {
    const res = await fetch(`${url}?tipo=cardapio`);
    const json = await res.json();
    const produtos = Array.isArray(json) ? json : (json.produtos || []);
    if (produtos.length) aplicarPrecos(produtos);
  } catch(e) {
    console.warn('Preços do Sheets indisponíveis:', e.message);
  }
}
...
function aplicarPrecos(produtos) {
  ...
  produtos.forEach(p => {
    ...
    mapa[chave] = { preco: p.preco, precoOriginal: p.precoOriginal||0, disponivel: p.disponivel };
  });
  Object.values(PRODUCTS).forEach(prod => {
    prod.sizes.forEach(size => {
      ...
      if (mapa[chave]) {
        const s = mapa[chave];
        if (s.precoOriginal > 0 && s.precoOriginal !== s.preco) {
          size.price = s.preco;
    ...
    // Disponibilidade: se TODAS as sizes indisponíveis, oculta o card
    const chavesDosProd = prod.sizes.map(s => (prod.name+'-'+s.label).toLowerCase().trim());
    const todasIndisponiveis = chavesDosProd.every(c => mapa[c] && mapa[c].disponivel === false);
    const card = document.querySelector(`.product-card[data-product="${prod.id}"]`);
    if (card) card.style.display = todasIndisponiveis ? 'none' : '';
  });
  atualizarTodosOsCards();
  renderPromocoes();
}
...
function renderPromocoes() {
  ...
  if (!sec) return;
  sec.style.display = '';
  sec.innerHTML = `<h2 class="prod-section-title promo-section-title">🔥 Promoções</h2>
    ${promos.map(prod => {
      ...
      return `<div class="product-card promo-destaque" data-product="${prod.id}" onclick="openProductModal('${prod.id}')">
...
function atualizarPrecoCard(card, prod) {
  ...
  if (temPromocao) {
    ...
    const prefixo = prod.sizes.length > 1 ? 'a partir de ' : '';
    priceEl.innerHTML = `<span class="price-old">${formatCurrency(precoOrig)}</span> `+
  ...
  sec.innerHTML = `
    ...
    <div class="monte-grid">
      ${ADICIONAIS.map(item => {
        ...
        return `<div class="monte-card ${qty>0?'monte-selected':''}">
    ...
    <div style="text-align:center;padding:16px;color:#9ca3af;font-size:.88rem;">👆 Selecione o tamanho do copo para continuar</div>`}
  `;
}
...
function addSelectedToCart() {
  ...
  else cart.push({key,productId:p.id,productName:p.name,size:p.sizes.length>1?s.label:null,price:s.price,qty:_selectedQty,image:p.image});
...
function changeQty(key,delta) {
  const i=cart.find(i=>i.key===key); if(!i) return;
  i.qty+=delta; if(i.qty<=0){removeFromCart(key);return;}
...
function doLogin() {
  ...
  currentUser=user; localStorage.setItem('tda_user',JSON.stringify(user));
  updateUserUI(); closeModal('modalAuth'); showToast(`Bem-vindo(a), ${user.name}! 🌿`);
...
function doRegister() {
  ...
  currentUser=user; localStorage.setItem('tda_user',JSON.stringify(user));
  updateUserUI(); closeModal('modalAuth'); showToast(`Bem-vindo(a), ${name}! 🌿`);
  _pollingAtivo=false; iniciarPollingStatus();
}
...
function openCheckout() {
  ...
  f('chkNeighborhood',currentUser.neighborhood); f('chkComplement',currentUser.complement);
...
function confirmOrder() {
  ...
  window._lastSubtotal=getSubtotal(); window._lastTotal=getTotal(); window._lastFreteGratis=_freteGratis;
...
function copyPixKey() {
  const k=PIX_KEY_FIXA||settings.pixKey;
  navigator.clipboard.writeText(k).then(()=>showToast('Chave PIX copiada!')).catch(()=>showToast('Não foi possível copiar.','error'));
}
...
async function verificarStatusPedidos() {
  ...
  try {
    const res=await fetch(`${url}?tipo=status_pedidos&telefone=${encodeURIComponent(currentUser.phone)}`);
    const json=await res.json();
...
function dispararNotificacao(pedido) {
  const st=ORDER_STATUSES[pedido.status]||{label:pedido.status};
  const msg=`Seu pedido está: ${st.label}`;
  showToast(`🔔 ${msg}`,'info');
  if('Notification' in window && Notification.permission==='granted') {
    try{new Notification('🌿 TDA Açaí',{body:msg,icon:'./Copo_thaina.png',tag:pedido.id});}catch(e){}
  }
}
    ...
    await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        pedido: {
          ...
          endereco: {rua:order.address.street,numero:order.address.number,bairro:order.address.neighborhood,complemento:order.address.complement||'',cidade:order.address.city},
          itens: order.items, pagamento: order.paymentMethod, troco: order.changeFor||'',
          subtotal: order.subtotal, taxaEntrega: order.deliveryFee, freteGratis: order.freteGratis||false,
          total: order.total, status: order.status, criadoEm: order.createdAt
        }
      })
    });
...
function atualizarStatusSheets() {
  ...
  el.textContent=url?'🟢 URL configurada':'⚪ Não configurado';
...
async function testarConexaoSheets() {
  ...
  try {
    const res=await fetch(`${url}?tipo=teste`);
    const json=await res.json();
    if(json.ok){
      showToast('✅ Conexão ok!','success');
      if(el){el.textContent='✅ Conectado';el.style.color='#16a34a';}
      settings.sheetsUrl=url; saveSettings();
    } else {
      showToast('⚠️ Resposta inesperada','error');
    }
  } catch(e) {
    showToast('❌ Não conectou. Verifique a URL.','error');
    if(el){el.textContent='❌ Falhou';el.style.color='#dc2626';}
  }
}
...
function renderAdminOrders() {
  ...
  el.innerHTML=orders.map(o=>{
    ...
    const taxaStr=o.freteGratis?'🎉 Grátis':formatCurrency(o.deliveryFee||0);
